...
if __name__=="__main__":
    modelmoves = [15,16,58,26,18,18,16,16,17,30]
    kwargtups = ((500,{'rar':0.98,'radr':0.999,'dyna':0}),(50,{'rar':0.5,'radr':0.99,'dyna':100}))
    for iterations,kwargs in kwargtups:
        print iterations, kwargs
        for worldnumber in range(1,11):
            minmoves = 500000
            results = []
            filename = 'testworlds/world%02d.csv' % worldnumber
            t = time.time()
    ...
    #            print iteration, "," , steps
                ...
                if steps < minmoves:
                    minmoves = steps
                    remdata = data
            print filename, "best", minmoves, "moves"
            print "acceptable", len([el for el in results if el <= modelmoves[worldnumber-1] * 1.5]), "/", iterations
            et = time.time()
            print "result time", str(et - t) + "s"
            printmap(remdata)
            print
